...
while True:
    ...
    if mode == "menu":
        ...
        y = 1
        for song in Song.songs:
            pg.draw.rect(screen, pg.Color(song.color), song.rect)
            text_rect = pg.rect.Rect(30, 100 * y + 5, song.text.get_rect().w, song.text.get_rect().h)
            text_rect.center = song.rect.center
            screen.blit(song.text, text_rect)
    ...
    if mode == "play":
        if is_play:  # только если сейчас идёт игра - нет проигрыша или победы
            ...
            if created_notes < len(playing_song.notes) and time.time() - timer >= playing_song.interval * playing_song.duration[created_notes]:
                if playing_song.duration[created_notes] == 1:
                    screen_notes.add(Tile(screen_notes))
                    timer = time.time()
                    created_notes += 1
                else:
                    screen_notes.add(Tile(screen_notes, long=True))
                    timer = time.time()
                    created_notes += 1
        ...
        # x = pg.image.load('long_tile.png')
        # screen.blit(x, (200, 0 + p))
        # p += 1
       # x.update()
        ...
        if failed:
            text = f1.render('Проигрыш', True, (255, 150, 255))
            text_rect = text.get_rect(center=(SIZE[0] // 2, SIZE[1] // 2))
            screen.blit(text, text_rect)   
